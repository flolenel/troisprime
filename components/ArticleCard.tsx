...
export function ArticleCard({ article, onTagClick }: ArticleCardProps) {
  ...
  return (
    ...
    >
      ...
      <Link href={`/articles/${slug.current}`} className="block md:hidden relative aspect-video overflow-hidden bg-brand-gray">
        {currentImage?.asset ? (
          <Image
          ...
          <div className="w-full h-full flex items-center justify-center" style={{ backgroundColor: config?.lightColor || '#F1F1F1' }}>
      ...
      <div className="p-4 flex flex-col flex-1 gap-2 min-w-0 md:max-w-[65%]">
        ...
        <Link href={`/articles/${slug.current}`}>
          <h2 className="font-extrabold text-brand-green text-lg leading-snug group-hover:text-orouge transition-colors duration-200 line-clamp-2">
            {title}
          </h2>
        </Link>
